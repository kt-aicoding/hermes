...
| [`scripts/scan-sensitive.py`](scripts/scan-sensitive.py) | 扫描当前仓库文件 / Git 历史中的敏感信息模式，只输出类型和位置 | 否 |

`optimize-cron-from-openclaw.py` 的默认行为是 dry-run。只有传入 `--apply` 时才会更新 Hermes cron 数据。报告会脱敏 delivery 目标，并避免打印完整 prompt。
...
扫描脚本只输出 finding 类型、文件和行号，不打印匹配值。
